...
class TeamController extends Controller
{
    ...
    public function StoreTeam(Request $request){
        ...
        ImageManager::imagick()->read($image)->resize(550,670)->save('upload/team/'.$name_gen);
    ...
    public function UpdateTeam(Request $request){
        ...
        if($request->file('image')){
            ...
            $image = $request->file('image');
            $name_gen = hexdec(uniqid()).'.'.$image->getClientOriginalExtension();
            ImageManager::imagick()->read($image)->save('upload/team/'.$name_gen);
            $save_url = 'upload/team/'.$name_gen;
    ...
    // ==================== Book Area All Methods =============
       ...
       public function BookAreaUpdate(Request $request){
        ...
        if($request->file('image')){
            ...
            $image = $request->file('image');
            $name_gen = hexdec(uniqid()).'.'.$image->getClientOriginalExtension();
            ImageManager::imagick()->read($image)->resize(1000,1000)->save('upload/bookarea/'.$name_gen);
